...
* NOTE: DONT UPDATE IF YOU USE V1! - Status on Migration to org-roam v2
...
There will be no smooth transition, however, The problem is that =Delve=
is heavily dependent on =Lister=, but =Lister= now just has been
rewritten. /Updating =Lister= will therefore cause that out-dated version
of =Delve= to break/. As a workaround, I have copied the legacy =Lister=
code into that repository.

That new variant, however, is completely untested. I couldn't even
start it because I do not have the old version of =org-roam= installed,
and creating a separate .emacs profile for that turned out to be too
much of a hazzle for me. 

In sum, if you keep on using =Delve= with the outdated version of
=org-roam=, you can:

 - Try that 'integrated' version and hope that it works.
 - If it does not work, use the previous commit and also install the
   =Lister= branch =archive-version-0.7.2= as a dependency.

The best thing, however, is simply to not upgrade.
